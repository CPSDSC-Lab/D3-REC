...
class AttrSCGM(nn.Module):
    """
    属性相似度计算引导模块
    使用ADM输出的属性权重（而非[CLS]token）计算视觉-属性相似度图
    """
    def __init__(self):
        super().__init__()
    
    def forward(self, vision_feature, text_features, attr_weights):
        """
        Args:
            vision_feature: (B, C, H, W) - 某一层的视觉特征
            text_features: (B, L, C) - BERT编码的文本特征（投影后256维）
            attr_weights: (B, L) - ADM输出的属性权重
        Returns:
            attr_similarity: (B, H, W) - 属性相似度图
        """
        b, c, h, w = vision_feature.shape
        
        # 展平视觉特征
        vision_flat = vision_feature.view(b, c, -1)  # (B, C, H*W)
        
        # 加权聚合属性token
        attr_weights_norm = attr_weights / (attr_weights.sum(dim=-1, keepdim=True) + 1e-8)  # (B, L)
        attr_repr = torch.bmm(attr_weights_norm.unsqueeze(1), text_features)  # (B, 1, C)
        
        # 计算相似度
        similarity = torch.bmm(attr_repr, vision_flat)  # (B, 1, H*W)
        similarity = similarity.view(b, h, w)  # (B, H, W)
        
        # 归一化到 [0, 1]
        sim_min = similarity.view(b, -1).min(dim=-1, keepdim=True)[0].unsqueeze(-1)
        sim_max = similarity.view(b, -1).max(dim=-1, keepdim=True)[0].unsqueeze(-1)
