...
from .attribute_decomposition_module import AttributeDecompositionModule
...
class GroundingDINOCountingGuide(nn.Module):
    ...
    def __init__(
        ...
        # Attribute Decomposition Module (ADM)
        self.adm = AttributeDecompositionModule(
    ...
    def forward(self, samples: NestedTensor, targets: List = None, **kw):
        ...
        # ADM 属性分解
        category_repr, attr_repr, cat_weights, adm_attr_weights, token_logits = self.adm(
            encoded_text, text_token_mask
        )
        
        # 将 ADM 输出存入 text_dict 供后续使用
        text_dict["category_repr"] = category_repr        # (B, 1, 256)
        text_dict["attr_repr"] = attr_repr                # (B, K, 256)
        text_dict["cat_weights"] = cat_weights            # (B, L)
        text_dict["adm_attr_weights"] = adm_attr_weights  # (B, L)
        text_dict["token_logits"] = token_logits          # (B, L, 3)
        
        # 获取 sd_attn_map（如果提供）并存入 text_dict 供 transformer 使用
        sd_attn_map = kw.get('sd_attn_map', None)
        if sd_attn_map is not None:
            text_dict['sd_attn_map'] = sd_attn_map
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal, img_embs, txt_embs, density, encode_regression_feature, sim_maps, attr_density = self.transformer( 
        ...
        # 如果启用了 DDG，返回 attr_density 供损失函数使用
        if hasattr(self.transformer, 'use_ddg') and self.transformer.use_ddg:
            out['attr_density'] = attr_density
        
        # ADM 输出（用于辅助损失）
        out['token_logits'] = text_dict['token_logits']           # (B, L, 3)
        out['category_repr'] = text_dict['category_repr']         # (B, 1, 256)
        out['attr_repr'] = text_dict['attr_repr']                 # (B, K, 256)
        out['adm_cat_weights'] = text_dict['cat_weights']         # (B, L)
        out['adm_attr_weights'] = text_dict['adm_attr_weights']   # (B, L)
        # 用于 ADM 辅助监督的 ground truth mask
        out['text_subject_mask'] = text_dict['text_subject_mask']     # (B, L)
        out['text_attribute_mask'] = text_dict['text_attribute_mask'] # (B, L)
        out['text_context_mask'] = text_dict['text_context_mask']     # (B, L)
