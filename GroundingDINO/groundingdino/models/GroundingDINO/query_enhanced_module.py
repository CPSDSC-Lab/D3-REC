...
class DualQueryLinguishDensityModule(nn.Module):
    """
    双路径查询增强：文本增强 + 类别密度增强 + 属性密度增强
    """
    def __init__(self, d_model=256, nhead=8):
        super().__init__()
        # 复用现有的文本增强模块
        self.text_enhance = QueryLinguishModule(d_model)
        # 类别密度增强（定位引导）
        self.cls_density_enhance = CrossAttentionLayer(d_model)
        # 属性密度增强（属性过滤）
        self.attr_density_enhance = CrossAttentionLayer(d_model)
    
    def forward(self, query, text_features, cls_density_features, attr_density_features,
                image_name=None, captions=None):
        """
        Args:
            query: (B, N, d_model) - 初始查询 (N=900)
            text_features: (B, L, d_model) - 文本特征
            cls_density_features: (B, N, d_model) - top-N的类别密度特征
            attr_density_features: (B, N, d_model) - top-N的属性密度特征
        Returns:
            enhanced_query: (B, N, d_model)
        """
        # 1. 文本增强
        query = self.text_enhance(query, text_features)
        # 2. 类别密度增强（WHERE引导）
        query = self.cls_density_enhance(query, cls_density_features)
        # 3. 属性密度增强（WHICH引导）
        query = self.attr_density_enhance(query, attr_density_features)
        return query
