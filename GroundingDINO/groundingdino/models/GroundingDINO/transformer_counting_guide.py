...
from .context_density_module import CCM, CACCM, MLCCM, AttrSCGM
from .coutning_attention import DensityAwareEnhance, DualDensityAwareEnhance
from .context_aware_module import CADM
from .counter import DensityRegressor, DualHeadDensityRegressor
from .query_enhanced_module import QueryLinguishModule, QueryLinguishDensityModule, DualQueryLinguishDensityModule
from .utils import (
    MLP,
    _get_activation_fn,
    _get_clones,
...
class Transformer(nn.Module):
    def __init__(
        ...
        fusion_dropout=0.1,
        fusion_droppath=0.0,
        # DDG/ADM/SDPR innovation switches
        use_ddg=False,
        use_adm=False,
        use_sdpr=False,
    ):
        ...
        # Conditional initialization for DDG innovations
        if self.use_ddg:
            # 属性相似度计算模块
            self.attr_scgm = AttrSCGM()
            
            # 双头密度回归器（共享骨干，分别输出类别密度和属性密度）
            self.dual_density_regressor = DualHeadDensityRegressor(
                counter_dim=256,
                use_sdpr=use_sdpr
            )
            
            # 双密度增强模块（类别密度→空间注意力，属性密度→通道注意力）
            self.dual_density_enhance = DualDensityAwareEnhance()
            
            # 双路径查询增强（文本 + 类别密度 + 属性密度）
            self.dual_query_enhance = DualQueryLinguishDensityModule(d_model=d_model)
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, text_dict=None, img_shape=None, image_name=None, captions=None):
        ...
        ## Compute attribute similarity maps (if DDG enabled)
        attr_sim_maps = []
        if self.use_ddg:
            # 从 text_dict 获取 ADM 属性权重
            attr_weights = text_dict.get('adm_attr_weights', None)
            if attr_weights is None:
                # fallback: 使用均匀权重
                attr_weights = text_dict['text_token_mask'].float()
            
            text_features = text_dict['encoded_text']
            
            # 对每个尺度计算属性相似度
            for i, src in enumerate(cnn):
                attr_sim = self.attr_scgm(src, text_features, attr_weights)
                attr_sim_maps.append(attr_sim)
        
        ## Density Regression
        if self.use_ddg:
            # 获取 sd_attn_map
            sd_attn_map = text_dict.get('sd_attn_map', None)
            
            # 双头密度回归
            cls_density, attr_density, density_hidden_features, sd_outputs = self.dual_density_regressor(
                cas_feature, cnn,
                cls_sim_maps=sim_maps,
                attr_sim_maps=attr_sim_maps,
                img_shape=img_shape,
                sd_attn_map=sd_attn_map,
                hidden_output=True
            )
            density = cls_density  # 主密度图仍用于返回
            
            # 计算属性密度的隐藏特征（用于查询增强）
            attr_density_hidden_features = density_hidden_features  # 共享特征
        else:
            ## Original: Density Regression 
        ...
        ## TODO: Density Feature Attention Enhance Module
            ...
            # Dual density enhancement (if DDG enabled) or original enhancement
            if self.use_ddg:
                # 双密度增强（类别密度→空间注意力，属性密度→通道注意力）
                cnn = self.dual_density_enhance(
                    cnn, density_hidden_features, attr_density_hidden_features
                )
            else:
                # 原始逻辑
        ...
        if self.two_stage_type == "standard": 
            ...
            # 如果启用了 DDG，计算属性密度的 topk 特征
            if self.use_ddg:
                attr_density_features_topk = torch.gather(
                    density_features, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, self.d_model)
                )
            else:
                attr_density_features_topk = None
        ...
        if self.density_warmup:
            ...
            if self.use_ddg:
                ...
                if self.query_detach:
                    ...
                    attr_density_feature_topk_detach = attr_density_features_topk.detach() if attr_density_features_topk is not None else None
                    tgt_enhanced = self.dual_query_enhance(
                        tgt, text_dict["encoded_text"],
                        density_feature_topk_detach,
                        attr_density_feature_topk_detach if attr_density_feature_topk_detach is not None else density_feature_topk_detach,
                        image_name, captions
                    )
                else:
                    tgt_enhanced = self.dual_query_enhance(
                        ...
                        attr_density_features_topk if attr_density_features_topk is not None else density_features_topk,
                        image_name, captions
                    )
            else:
                # 原始逻辑
        ...
        return hs, references, hs_enc, ref_enc, init_box_proposal, img_embs, txt_embs, density, output_memory_regression_dict, sim_maps, attr_density
    ...
    def forward(
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.fusion_layers:
                if self.use_checkpoint and self.training: # 更新vision feature和text feature
...
def build_transformer(args):
    return Transformer(
        ...
        # DDG/ADM/SDPR innovation switches
        use_ddg=getattr(args, 'use_ddg', False),
        use_adm=getattr(args, 'use_adm', False),
        use_sdpr=getattr(args, 'use_sdpr', False),
    )
